...
import DonationItemForm from "~/components/items/donation-item-form";
...
export default function CreateDonationItemPage() {
  ...
  const uploadImageAfterCreate = async (itemId: string, file: File) => {
    ...
    try {
      console.log('Mengupload gambar untuk item donasi baru dengan ID:', itemId);
  ...
  const handleSubmit = (data: NewItem) => {
    ...
    createItem.mutate(itemData, {
      onSuccess: async (result) => {
        showToast("Item donasi berhasil ditambahkan", "success");
      ...
      onError: (err) => {
        setError(err.message || "Gagal menambahkan item donasi");
        showToast(err.message || "Gagal menambahkan item donasi", "error");
      }
    });
  };
  ...
  return (
    <div class="px-4 py-6 max-w-2xl mx-auto">
      <div class="flex flex-col items-center gap-2 mb-6">
        <h1 class="text-2xl font-bold text-sidebar text-center">Tambah Barang Donasi</h1>
        <button 
      ...
      <div class="bg-white rounded-lg shadow-md border p-6">
        <DonationItemForm 
          ...
          submitLabel={isUploadingImage() ? "Mengupload Gambar..." : "Tambah Barang Donasi"}
