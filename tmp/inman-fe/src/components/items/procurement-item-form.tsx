...
export default function ProcurementItemForm(props: ProcurementItemFormProps) {
  const categoriesQuery = useCategories();
  const conditionsQuery = useConditions();
  const locationsQuery = useLocations();
  const sourcesQuery = useSources();
  
  // State untuk menyimpan ID sumber pengadaan
  const [procurementSourceId, setProcurementSourceId] = createSignal<UUID | undefined>(undefined);
  
  // Ambil ID sumber "Pengadaan" dari API
  createEffect(() => {
    if (sourcesQuery.data) {
      console.log("Sources data loaded:", sourcesQuery.data);
      const procurementSource = sourcesQuery.data.find((source: ItemSource) => 
        source.name.toLowerCase() === "pengadaan");
      
      console.log("Found procurement source:", procurementSource);
      
      if (procurementSource) {
        // Pastikan ID memiliki format UUID yang benar
        setProcurementSourceId(procurementSource.id as UUID);
        // Update formData dengan source_id (wajib di backend)
        setFormData(prev => ({ ...prev, source_id: procurementSource.id as UUID }));
        console.log("Updated form data with source_id:", procurementSource.id);
      } else {
        // Jika tidak menemukan sumber "Pengadaan", gunakan sumber pertama yang tersedia
        if (sourcesQuery.data.length > 0) {
          const firstSource = sourcesQuery.data[0];
          setProcurementSourceId(firstSource.id as UUID);
  ...
  const [formData, setFormData] = createSignal<Partial<NewItem>>(props.initialData || {
    ...
    donor_id: undefined, // Tidak digunakan untuk pengadaan
    procurement_id: undefined, // Field ini khusus untuk pengadaan, akan diisi oleh user
  ...
  const handleSubmit = (e: Event) => {
    ...
    console.log("Procurement source ID:", procurementSourceId());
    
    // Periksa apakah source_id sudah tersedia (wajib di backend)
    if (!formData().source_id && !procurementSourceId()) {
      console.error("ID sumber pengadaan belum tersedia, menunggu data dari API");
      return;
    }
    
    // Jika source_id tidak ada di formData tapi procurementSourceId ada, gunakan procurementSourceId
    if (!formData().source_id && procurementSourceId()) {
      setFormData(prev => ({ ...prev, source_id: procurementSourceId() }));
    }
    ...
    console.log('Submitting procurement item with data:', finalData);
  ...
  return (
    <form class="space-y-4" onSubmit={handleSubmit}>
      ...
      {/* Field khusus untuk pengadaan */}
      <div>
        <label class="font-semibold block mb-1">ID Pengadaan</label>
        <input 
          name="procurement_id" 
          class="border rounded text-sm px-3 py-2 w-full" 
          value={formData().procurement_id || ""} 
          onInput={handleChange} 
          placeholder="Masukkan ID atau referensi pengadaan"
        />
        <p class="text-xs text-gray-500 mt-1">ID atau referensi pengadaan untuk barang ini</p>
      </div>
      ...
      <div class="flex justify-end gap-2 pt-2">
        ...
        <button 
          ...
          disabled={props.isSubmitting || (!formData().source_id && !procurementSourceId())}
          onClick={() => {
            // Debug button click
            console.log("Button clicked, form data:", formData());
            console.log("Button clicked, procurement source ID:", procurementSourceId());
          }}
        >
